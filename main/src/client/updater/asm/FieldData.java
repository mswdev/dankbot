...
public class FieldData {
    ...
    private boolean doArgsMatch(FieldSigniture ms, String[] args) {
        for (int i = 0; i < ms.arguments.length; ++i) {
            ...
            if (ms.arguments[i].startsWith("[") && args[i].startsWith("[")) {
                continue;
            }
            if (!ms.arguments[i].equalsIgnoreCase(args[i])) {
                return false;
            }
        }
        return true;
    ...
    public boolean isReferenceMatch(FieldSigniture fs) {
        for (MethodData mn : referencedFrom) {
            if (fs.returnType.equals("?") || fs.returnType.equals(mn.bytecodeMethod.desc.substring(mn.bytecodeMethod.desc.indexOf(")") + 1, mn.bytecodeMethod.desc.length()))) {
                String[] args = parseArguments(mn.bytecodeMethod);
                if (fs.arguments.length == args.length && doArgsMatch(fs, args)) {
